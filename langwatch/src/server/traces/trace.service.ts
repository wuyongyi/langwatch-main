    ...
    return this.tracer.withActiveSpan(
      ...
      async (span) => {
        ...
        if (useClickHouse) {
          ...
          // ClickHouse doesn't include evaluations, fetch them separately
          if (trace) {
            const evaluationsMap = await this.getEvaluationsMultiple(
              projectId,
              [traceId],
              protections,
            );
            trace.evaluations = evaluationsMap[traceId] ?? [];
          }
        } else {
          // Elasticsearch includes evaluations when includeEvaluations: true
          trace = await this.elasticsearchService.getById(
            projectId,
            traceId,
            protections,
          );
