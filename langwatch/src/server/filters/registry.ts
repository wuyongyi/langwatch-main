...
export const availableFilters: { [K in FilterField]: FilterDefinition } = {
  ...
  "metadata.risk_level": {
    ...
    urlKey: "risk_level",
    query: (values: string[]) => ({
      terms: { "metadata.langwatch.risk.level": values },
    }),
    listMatch: {
      aggregation: (query: string | undefined) => ({
        unique_values: {
          ...
          aggs: {
            child: {
              terms: {
                field: "metadata.langwatch.risk.level",
                size: 10_000,
                order: {
                  _key: "asc",
                },
      ...
      extract: (result: Record<string, any>) => {
        const buckets = result.unique_values?.child?.buckets?.map((bucket: any) => {
          const riskLevel = bucket.key;
          let label = riskLevel;
          if (riskLevel === "low") label = "Low Risk";
          else if (riskLevel === "medium") label = "Medium Risk";
          else if (riskLevel === "high") label = "High Risk";
        ...
        return buckets.sort((a: any, b: any) => {
          ...
          return (order[a.field] || 999) - (order[b.field] || 999);
        });
      },
    },
  },
