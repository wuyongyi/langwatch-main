...
> = {
  ...
  "metadata.risk_level": {
    ...
    buildQuery: (params) => {
      ...
      return `
        SELECT
          ts.${ATTRIBUTE_KEYS.risk_level} as field,
          CASE ts.${ATTRIBUTE_KEYS.risk_level}
            WHEN 'low' THEN 'Low Risk'
            WHEN 'medium' THEN 'Medium Risk'
            WHEN 'high' THEN 'High Risk'
            ELSE ts.${ATTRIBUTE_KEYS.risk_level}
          END as label,
          count() as count
        FROM trace_summaries ts
        WHERE ${buildTraceSummariesConditions(params)}
          AND ts.${ATTRIBUTE_KEYS.risk_level} != ''
          ${buildQueryFilter(`ts.${ATTRIBUTE_KEYS.risk_level}`, params)}
          ${scopeSql}
        GROUP BY field
        ORDER BY 
          CASE field
