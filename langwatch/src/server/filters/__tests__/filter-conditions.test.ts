...
describe("clickHouseFilterConditions", () => {
  ...
  describe("metadata filters", () => {
    ...
    it("generates risk_level filter with correct attribute path", () => {
      const builder = clickHouseFilterConditions["metadata.risk_level"];
      expect(builder).not.toBeNull();
      const result = builder!(["high", "medium"], "f0");
      expect(result.sql).toBe(
        "ts.Attributes['langwatch.risk.level'] IN ({f0_values:Array(String)})"
      );
      expect(result.params).toEqual({ f0_values: ["high", "medium"] });
    });
  });
