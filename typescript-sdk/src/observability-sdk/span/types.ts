...
export interface LangWatchSpan extends Span {
  ...
  /**
   * Set custom metadata on the span.
   *
   * Metadata is stored as attributes with the prefix "langwatch.metadata."
   * Supports string, number, boolean, and array value types.
   *
   * @param key - The metadata key
   * @param value - The metadata value (string, number, boolean, or array)
   * @returns this
   *
   * @example
   * span.setMetadata("environment", "production");
   * span.setMetadata("retry_count", 3);
   * span.setMetadata("is_test", false);
   * span.setMetadata("tags", ["urgent", "customer-facing"]);
   */
  setMetadata(
    key: string,
   ...
   * });
   */
  setMetadataBatch(metadata: Record<string, string | number | boolean | string[] | number[] | boolean[]>): this;
