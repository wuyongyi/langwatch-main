...
// --- Agent Trace Creation Tools (OTLP-based) ---

server.tool(
  "create_trace",
  `Create a new trace for agent execution using OTLP protocol. Returns trace_id for reference.

This tool creates a root trace that will appear in LangWatch. To add child spans during execution, use the SDK instrumentation (@langwatch.span decorator) in your agent code rather than MCP tools.

Use cases:
- Start tracking a new agent task
- Create a trace before executing agent logic
- Initialize observability for a user request`,
  {
    name: z.string().describe("Trace name (e.g., 'user-query', 'code-generation')"),
    input: z.string().describe("Agent input or user request"),
    metadata: z
      .object({
        user_id: z.string().optional().describe("User identifier"),
        thread_id: z.string().optional().describe("Conversation thread ID"),
        task_type: z.string().optional().describe("Task classification"),
      })
      .optional()
      .describe("Optional metadata for filtering and grouping"),
  },
  async (params) => {
    const { requireApiKey } = await import("./config.js");
    requireApiKey();
...
server.tool(
  "record_evaluation",
  `Record an evaluation result for a trace using OTLP protocol.

Use this to attach evaluation results (pass/fail, scores, details) to existing traces. Evaluations help assess agent output quality and track performance over time.

Use cases:
- Record automated evaluator results
- Log manual review outcomes
- Track quality metrics for agent responses`,
  {
    trace_id: z.string().describe("The trace ID to attach evaluation to"),
    evaluator_name: z
      .string()
      .describe("Evaluator identifier (e.g., 'code-correctness', 'response-accuracy')"),
    passed: z.boolean().describe("Whether the evaluation passed"),
    score: z
      .number()
      .optional()
      .describe("Numeric score (0.0 to 1.0)"),
    details: z
      .string()
      .optional()
      .describe("Additional evaluation details or feedback"),
  },
  async (params) => {
    const { requireApiKey } = await import("./config.js");
    requireApiKey();
    const { handleRecordEvaluation } = await import(
      "./tools/record-evaluation.js"
    );
    return {
      content: [
        { type: "text", text: await handleRecordEvaluation(params) },
      ],
    };
  }
);
